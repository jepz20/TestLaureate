angular.module('TestLaureateApp').filter('textoNoExacto', [function () {
    return function( arreglo, filtro, val) {
        ...
        if (filtro) {
            if (filtro.title || filtro.title !=='') {
                for (var a = 0; a < arreglo.length; a++) {
                    var indiceComparativo, indiceRepetido;
                    var comparativoFiltro = filtro.title;
                    var comparativoPrograma = arreglo[a].snippet.title;
                    ...
                    if (valorMatch !== 0) {
                        //console.log(arreglo[a].title, valorMatch, totalPosicion,totalDistancia, comparativoPrograma.length);
